...
export function ModelsBrowserClient({ providers }: { providers: ProviderRow[] }) {
  ...
  const allModels = useMemo(() => {
    ...
    const list: (ModelRow & { providerName: string })[] = [];
    providers.forEach(p => {
      p.models.forEach(m => {
        if (!q || m.modelId.toLowerCase().includes(q) || m.displayName.toLowerCase().includes(q)) {
          list.push({ ...m, providerName: p.providerName });
        }
      });
    });
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {allModels.map((m) => (
            ...
            >
              ...
              {m.description && (
                <p className="text-xs text-text-muted line-clamp-2 h-8">{m.description}</p>
              )}
